...
class Column extends ValueGroup
{
    ...
    public function replaceValuesbyColumn(Column $other): void
    {
        $thisFields = $this->getFields();
        $otherFields = $other->getFields();

        if (sizeof($thisFields) != sizeof($otherFields)){
            throw new Exception("Cannot replace the values by a column of unequal length (this length " . sizeof($thisFields) . ", other length ". sizeof($otherFields) . ")");
        }
